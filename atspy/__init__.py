        ...
        if m == "HWAMS":
            try:
                ...
                try:
                    ...
                    models_dict[m] = ExponentialSmoothing(
                        train, seasonal_periods=seasons, trend=None, seasonal="add"
                    ).fit(use_boxcox=False)
        ...
        if name == "Gluonts":
            ...
            if in_sample:
                for df_entry, forecast in zip(
                    gluonts_dataframe(df), model.predict(gluonts_dataframe(df))
                ):
                ...
                for df_entry, forecast in zip(
                    ...
                    forecast_dict[name] = forecast.samples.mean(
                        axis=0
                    )  # .quantile(0.5)
