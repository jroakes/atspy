...
def plot(
    ...
    if uncertainty and m.uncertainty_samples:
        ax.fill_between(
            fcst_t, fcst["yhat_lower"], fcst["yhat_upper"], color="#0072B2", alpha=0.2
        )
...
def plot_seasonality(m, name, ax=None, uncertainty=True, figsize=(10, 6)):
    ...
    if period <= 2:
        ...
        fmt_str = "{dt:%m}/{dt:%d}"
    ax.xaxis.set_major_formatter(
        FuncFormatter(lambda x, pos=None: fmt_str.format(dt=num2date(x)))
    )
    ax.set_xlabel("ds")
    ax.set_ylabel(name)
    if m.seasonalities[name]["mode"] == "multiplicative":
...
def plot_cross_validation_metric(
    ...
    x_plt = df_none["horizon"].astype("timedelta64[ns]").astype(np.int64) / float(
        dt_conversions[i]
    )
    x_plt_h = df_h["horizon"].astype("timedelta64[ns]").astype(np.int64) / float(
        dt_conversions[i]
    )
